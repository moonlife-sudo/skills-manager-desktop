...
export function WorkspaceView({ config }: { config: WorkspaceConfig }) {
  ...
  const { tools, managedSkills, presets, refreshManagedSkills, refreshTools, openSkillDetailById } = useApp();
  ...
  const openLocalDetail = useCallback(
    async (skill: ProjectSkill) => {
      if (!currentTool) return;
      if (skill.center_skill_id && managedLocalIds.has(skill.center_skill_id)) {
        openSkillDetailById(skill.center_skill_id);
        return;
      }
    ...
    [currentTool, managedLocalIds, openSkillDetailById]
  );
