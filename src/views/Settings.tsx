...
import type { ReadabilityMode, Theme } from "../hooks/useTheme";
...
function previewMediaSrc(path: string) {
  if (!path) return "";
...
export function Settings() {
  ...
  const { theme, setTheme, appearance, setAppearanceValue, resetAppearance } = useThemeContext();
  ...
  const [importingBackgroundMedia, setImportingBackgroundMedia] = useState(false);
  const [backgroundMediaError, setBackgroundMediaError] = useState("");
  ...
  const handleSelectBackgroundMedia = async () => {
    const isVideo = appearance.backgroundKind === "video";
    try {
      const selected = await dialogOpen({
        multiple: false,
        filters: isVideo
          ? [{ name: "MP4 videos", extensions: ["mp4", "m4v", "webm"] }]
          : [{ name: "Images", extensions: ["png", "jpg", "jpeg", "webp", "gif"] }],
      });
      if (typeof selected === "string") {
        setImportingBackgroundMedia(true);
        setBackgroundMediaError("");
        const importedPath = await api.importAppearanceMedia(selected, appearance.backgroundKind);
        setAppearanceValue("backgroundPath", importedPath);
        toast.success(isVideo ? "Video wallpaper imported" : "Wallpaper imported");
      }
    } catch (error) {
      const message = getErrorMessage(error, "Failed to import wallpaper media");
      setBackgroundMediaError(message);
      toast.error(message);
    } finally {
      setImportingBackgroundMedia(false);
    }
  };

  const handleBackgroundKindChange = (kind: "image" | "video") => {
    setBackgroundMediaError("");
    setAppearanceValue("backgroundKind", kind);
    if (
      appearance.backgroundPath &&
      ((kind === "image" && /\.(mp4|m4v|webm)$/i.test(appearance.backgroundPath)) ||
        (kind === "video" && !/\.(mp4|m4v|webm)$/i.test(appearance.backgroundPath)))
    ) {
      setAppearanceValue("backgroundPath", "");
    }
  };
  ...
  const handleAddPet = useCallback(() => {
    setPets((current) => [...current, createDefaultPet()]);
  }, [setPets]);
  ...
  const themeOptions: Array<{ value: Theme; label: string; icon: typeof Sun }> = [
    ...
    { value: "sakura", label: "Sakura", icon: Sparkles },
    { value: "anime", label: "Anime", icon: Palette },
    { value: "cyber", label: "Cyber", icon: Zap },
    { value: "soft", label: "Soft", icon: Sun },
  ];
  const readabilityOptions: Array<{ value: ReadabilityMode; label: string; description: string; icon: typeof Sun }> = [
    { value: "readable", label: "Clear", description: "Stronger veil and solid panels for daily work.", icon: BookOpen },
    { value: "balanced", label: "Balanced", description: "Keeps the wallpaper visible while protecting text.", icon: Sparkles },
    { value: "immersive", label: "Immersive", description: "More wallpaper, lighter panels.", icon: Zap },
  ];
  ...
  const renderAgentCard = (agent: typeof tools[number], dragHandle?: React.ReactNode) => (
    ...
    >
      <div className="flex items-center gap-2">
        <div className="flex h-7 w-4 shrink-0 items-center justify-center">{dragHandle}</div>
          ...
          <div className="overflow-hidden rounded-xl border border-border-subtle bg-surface divide-y divide-border-subtle">
            ...
            <div className="px-4 py-3">
              <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <h3 className="text-[13px] text-secondary font-medium mb-0.5">Appearance</h3>
                  <p className="text-[13px] text-muted">Customize image or MP4 video wallpaper and workspace readability.</p>
                </div>
                <button
                  type="button"
                  onClick={handleResetAppearance}
                  className={cn(actionButtonClass, "border-border-subtle bg-background text-muted hover:text-secondary")}
                >
              ...
              <div className="space-y-3 rounded-lg border border-border-subtle bg-background p-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex rounded-[4px] border border-border-subtle bg-surface p-px">
                    <button
                      type="button"
                      onClick={() => handleBackgroundKindChange("image")}
                      className={cn(
                        segmentedButtonClass,
                        appearance.backgroundKind === "image" ? "bg-surface-active text-secondary" : "text-muted hover:text-tertiary"
                      )}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => handleBackgroundKindChange("video")}
                      className={cn(
                        segmentedButtonClass,
                        appearance.backgroundKind === "video" ? "bg-surface-active text-secondary" : "text-muted hover:text-tertiary"
                      )}
                    >
                  ...
                  {appearance.backgroundKind === "video" ? (
                    <button
                      type="button"
                      onClick={() => {
                        const nextMuted = !appearance.videoMuted;
                        setAppearanceValue("videoMuted", nextMuted);
                        if (!nextMuted && appearance.videoVolume <= 0) {
                          setAppearanceValue("videoVolume", 0.5);
                        }
                      }}
                      className={cn(actionButtonClass, "border-border-subtle bg-surface text-secondary hover:bg-surface-hover")}
                    >
                      {appearance.videoMuted ? <VolumeX className="h-3.5 w-3.5" /> : <Volume2 className="h-3.5 w-3.5" />}
                      {appearance.videoMuted ? "Muted" : "Sound on"}
                    </button>
                  ) : null}
                </div>
                ...
                <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
                  <div className="flex min-w-0 flex-1 items-center gap-2">
                    ...
                    <input
                      value={appearance.backgroundPath}
                      onChange={(event) => {
                        setBackgroundMediaError("");
                        setAppearanceValue("backgroundPath", event.target.value);
                      }}
                      placeholder={appearance.backgroundKind === "video" ? "Background MP4 video path" : "Background image path"}
                      className="h-8 min-w-0 flex-1 rounded-[4px] border border-border-subtle bg-surface px-2.5 text-[13px] text-secondary outline-none transition-colors focus:border-border"
                    />
                  </div>
                  <div className="flex shrink-0 gap-2">
                    <button
                      type="button"
                      onClick={handleSelectBackgroundMedia}
                      disabled={importingBackgroundMedia}
                      className={cn(actionButtonClass, "border-border-subtle bg-surface text-secondary hover:bg-surface-hover")}
                    >
                      {importingBackgroundMedia ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FolderOpen className="h-3.5 w-3.5" />}
                      {importingBackgroundMedia ? "Importing" : "Select"}
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className={cn(actionButtonClass, "border-border-subtle bg-surface text-muted hover:text-secondary")}
                    >
                ...
                <div className="overflow-hidden rounded-lg border border-border-subtle bg-surface">
                  <div className="relative h-36">
                    {appearance.backgroundPath ? (
                      appearance.backgroundKind === "video" ? (
                        <video
                          src={previewMediaSrc(appearance.backgroundPath)}
                          className="absolute inset-0 h-full w-full object-cover"
                          autoPlay
                          loop
                          playsInline
                          muted={appearance.videoMuted}
                          onError={() => setBackgroundMediaError("Video preview failed. Re-select the file to import it into the app media folder.")}
                          onLoadedData={() => setBackgroundMediaError("")}
                          style={{
                        ...
                        <img
                          ...
                          alt=""
                          className="absolute inset-0 h-full w-full object-cover"
                          onError={() => setBackgroundMediaError("Image preview failed. Re-select the file to import it into the app media folder.")}
                          onLoad={() => setBackgroundMediaError("")}
                          style={{
                            opacity: appearance.backgroundOpacity,
                            filter: `blur(${appearance.backgroundBlur}px)`,
                            transform: "scale(1.04)",
                    ...
                    {backgroundMediaError ? (
                      <div className="absolute inset-x-4 top-4 rounded-md border border-amber-300/50 bg-bg-primary/90 px-3 py-2 text-[12px] text-amber-600 shadow-sm backdrop-blur">
                        {backgroundMediaError}
                      </div>
                    ) : null}
                    <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-3 bg-surface/80 px-3 py-2 backdrop-blur">
                      <span className="truncate text-[12px] text-muted">
                        {appearance.backgroundPath ? compactHomePath(appearance.backgroundPath) : "No wallpaper selected"}
                      </span>
                      <span className="shrink-0 rounded-full border border-border-subtle bg-bg-secondary px-2 py-0.5 text-[11px] text-muted">
                        Preview
                      </span>
                    </div>
                  </div>
                ...
                <div className="grid gap-2 md:grid-cols-3">
                  {readabilityOptions.map((option) => {
                    ...
                    const active = appearance.readabilityMode === option.value;
                    return (
                      <button
                        key={option.value}
                        type="button"
                        onClick={() => setAppearanceValue("readabilityMode", option.value)}
                        className={cn(
                          "flex min-h-[76px] items-start gap-2 rounded-lg border px-3 py-2.5 text-left transition-colors outline-none",
                          active
                      ...
                      >
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="mt-1 block text-[12px] leading-4 text-muted">{option.description}</span>
                        </span>
                      </button>
                    );
                  })}
                ...
                {appearance.backgroundKind === "video" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    {[
                      {
                        key: "videoPlaybackRate" as const,
                        label: "Playback speed",
                        min: 0.25,
                        max: 3,
                        step: 0.05,
                        value: appearance.videoPlaybackRate,
                        suffix: "x",
                      },
                      {
                        ...
                        max: 1,
                        step: 0.01,
                        value: appearance.videoVolume,
                        suffix: "",
                        disabled: appearance.videoMuted,
                      },
                    ].map((item) => (
                      <label key={item.key} className={cn("min-w-0", item.disabled && "opacity-50")}>
                        <span className="mb-1 flex items-center justify-between gap-2 text-[12px] text-muted">
                          <span>{item.label}</span>
                          <span className="font-mono text-faint">
                            {item.suffix === "x" ? `${item.value.toFixed(2)}x` : item.value.toFixed(2)}
                          </span>
                        </span>
                        <input
                          type="range"
                          ...
                          onChange={(event) => setAppearanceValue(item.key, Number(event.target.value))}
                  ...
                  {[
                    ...
                    <label key={item.key} className="min-w-0">
                      <span className="mb-1 flex items-center justify-between gap-2 text-[12px] text-muted">
                        <span>{item.label}</span>
                        <span className="font-mono text-faint">
                          {item.key === "backgroundBlur" ? `${item.value}px` : item.value.toFixed(2)}
                        </span>
                      </span>
                      <input
                        type="range"
            ...
            <div className="px-4 py-3">
              <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
                ...
                <button
                  ...
                  className={cn(actionButtonClass, "border-accent-border bg-accent-dark text-white hover:bg-accent")}
                >
              ...
              <div className="space-y-3 rounded-lg border border-border-subtle bg-background p-3">
                {pets.length === 0 ? (
                  ...
                  pets.map((pet, index) => (
                    <div key={pet.id} className="rounded-lg border border-border-subtle bg-surface p-3">
                      <div className="mb-3 flex flex-wrap items-center gap-2">
                        <button
                          ...
                          className={cn(
                            ...
                            pet.enabled ? "bg-emerald-500" : "bg-zinc-300 dark:bg-zinc-600"
                          )}
                        >
                          <span
                            className={cn(
                              "h-4 w-4 rounded-full bg-white shadow transition-transform",
                              pet.enabled ? "translate-x-4" : "translate-x-0.5"
                            )}
                          />
                        </button>
                        <input
                        ...
                        <button
                          ...
                          className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[4px] border border-border-subtle bg-background text-muted hover:text-red-500"
                          title="Remove pet"
                        >
