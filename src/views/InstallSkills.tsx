...
import { MarketSkillPreviewDialog } from "../components/MarketSkillPreviewDialog";
...
export function InstallSkills() {
  ...
  const [marketPreviewSkill, setMarketPreviewSkill] = useState<SkillsShSkill | null>(null);
  ...
  useEffect(() => {
    ...
    if (!loadingMore) {
      const cacheKey = query.length > 0
        ? `search:${query.toLowerCase()}|${marketSearchLimit}`
        : `leaderboard:${marketTab}`;
  ...
  const handleInstallSkillssh = async (skill: SkillsShSkill, options: MarketInstallOptions = {}) => {
    ...
    try {
      ...
      await api.installFromSkillssh(skill.source, skill.skill_id, options.name);

      const nextSkills = await api.getManagedSkills();
      const installedSkill = nextSkills.find(
        (item) => item.source_type === "skillssh" && item.source_ref === cancelKey
      );

      const postInstallTasks: Promise<unknown>[] = [];
      if (installedSkill && options.tags && options.tags.length > 0) {
        postInstallTasks.push(api.setSkillTags(installedSkill.id, options.tags));
      }
      if (installedSkill && options.presetId) {
        postInstallTasks.push(api.addSkillToPreset(installedSkill.id, options.presetId));
      }
      if (postInstallTasks.length > 0) {
        await Promise.all(postInstallTasks);
      }

      await Promise.all([refreshPresets(), refreshManagedSkills()]);
      if (options.openAfterInstall !== false && installedSkill) {
        openSkillDetailById(installedSkill.id);
        navigate("/my-skills");
      }
      setMarketPreviewSkill(null);
      toast.success(t("install.toast.success", { name: options.name || displayName }), {
        id: toastId,
        action: {
          label: t("install.toast.view"),
          onClick: () => {
            if (installedSkill) {
              ...
              goToSkill(options.name || displayName);
            }
          },
        },
      });
  ...
  return (
    <div className="app-page gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-3">
                    {paginatedMarketSkills.map((skill) => {
                      ...
                      <div
                        ...
                        role="button"
                        tabIndex={0}
                        onClick={() => setMarketPreviewSkill(skill)}
                        onKeyDown={(event) => {
                          if (event.key === "Enter" || event.key === " ") {
                            event.preventDefault();
                            setMarketPreviewSkill(skill);
                          }
                        }}
                        className="app-panel flex cursor-pointer flex-col gap-2 p-3 transition-colors hover:border-border"
                      >
      ...
      <MarketSkillPreviewDialog
        skill={marketPreviewSkill}
        installed={
          marketPreviewSkill
            ? installedSourceRefs.has(`${marketPreviewSkill.source}/${marketPreviewSkill.skill_id}`)
            : false
        }
        installing={Boolean(marketPreviewSkill && installing === marketPreviewSkill.id)}
        presets={presets}
        defaultPresetId={viewedPreset?.id ?? null}
        onClose={() => setMarketPreviewSkill(null)}
        onInstall={(targetSkill, options) =>
          handleInstallSkillssh(targetSkill, {
            ...options,
