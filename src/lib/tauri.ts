...
export interface MarketSkillPreview {
  ...
  description: string | null;
  files: SkillFileNode[];
  document: SkillFileContent | null;
  file_previews: MarketSkillFilePreview[];
  risk_issues: SkillQualityIssue[];
}

export interface MarketSkillFilePreview {
  relative_path: string;
  content: string;
  size: number;
  hash: string;
...
export interface SkillEditSnapshot {
  id: string;
  ts: number;
  skill_id: string;
  skill_name: string;
...
export const getSkillFileTree = (skillId: string) =>
  invoke<SkillFileNode[]>("get_skill_file_tree", { skillId });

export const readSkillFile = (skillId: string, relativePath: string) =>
  invoke<SkillFileContent>("read_skill_file", { skillId, relativePath });

export const previewSkillFileSave = (
  skillId: string,
  relativePath: string,
  updatedText: string,
  originalHash: string,
) =>
  invoke<SkillFileDiff>("preview_skill_file_save", {
    skillId,
    relativePath,
    updatedText,
    originalHash,
...
export const saveSkillFile = (
  ...
  invoke<ManagedSkill>("save_skill_file", {
    skillId,
    relativePath,
    updatedText,
    originalHash,
...
export const checkSkillQuality = (skillId: string) =>
  invoke<SkillQualityIssue[]>("check_skill_quality", { skillId });

export const exportSkillArchive = (skillId: string) =>
  invoke<string>("export_skill_archive", { skillId });

export const getSkillAuditHistory = (skillId: string) =>
  invoke<SkillAuditEntry[]>("get_skill_audit_history", { skillId });

export const listSkillEditSnapshots = (skillId: string) =>
  invoke<SkillEditSnapshot[]>("list_skill_edit_snapshots", { skillId });

export const restoreSkillEditSnapshot = (skillId: string, snapshotId: string) =>
  invoke<ManagedSkill>("restore_skill_edit_snapshot", { skillId, snapshotId });
...
export const installFromSkillssh = (source: string, skillId: string, name?: string | null) =>
  invoke<void>("install_from_skillssh", { source, skillId, name: name || null });

export const previewSkillsshSkill = (source: string, skillId: string) =>
  invoke<MarketSkillPreview>("preview_skillssh_skill", { source, skillId });
...
export const importAppearanceMedia = (path: string, kind: "image" | "video") =>
  invoke<string>("import_appearance_media", { path, kind });
