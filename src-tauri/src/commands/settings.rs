...
#[tauri::command]
pub async fn import_appearance_media(
    app: tauri::AppHandle,
    path: String,
    kind: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if source.as_os_str().is_empty() {
            return Err(AppError::invalid_input("Media path is empty"));
        }
        if !source.is_file() {
            return Err(AppError::not_found("Selected media file does not exist"));
        }
        ...
        let extension = source
            ...
            .ok_or_else(|| AppError::invalid_input("Selected media file has no extension"))?;

        let allowed = match kind.as_str() {
            "image" => ["png", "jpg", "jpeg", "webp", "gif"].contains(&extension.as_str()),
            "video" => ["mp4", "m4v", "webm"].contains(&extension.as_str()),
            _ => return Err(AppError::invalid_input("Unsupported media kind")),
        };
        if !allowed {
            return Err(AppError::invalid_input(format!(
                "Unsupported {kind} file extension: .{extension}"
            )));
        }

        let metadata = fs::metadata(&source).map_err(AppError::io)?;
        let max_size = if kind == "video" {
            500 * 1024 * 1024
        } else {
            50 * 1024 * 1024
        };
        if metadata.len() > max_size {
            return Err(AppError::invalid_input(format!(
                "Selected file is too large for a {kind} wallpaper"
            )));
        }
        ...
        let timestamp = SystemTime::now()
            ...
            .map_err(AppError::internal)?
            .as_millis();
        let destination = media_dir.join(format!("{stem}-{timestamp}.{extension}"));

        fs::copy(&source, &destination).map_err(AppError::io)?;
        Ok(destination.to_string_lossy().to_string())
    })
    .await?
}

fn sanitize_media_stem(stem: &str) -> String {
    stem.chars()
        .map(|ch| {
